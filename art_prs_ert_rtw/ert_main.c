...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(art_prs_M) == (NULL)) && !rtmGetStopRequested
    (art_prs_M);
  while (runModel) {
    ...
    stopRequested = !((rtmGetErrorStatus(art_prs_M) == (NULL)) &&
                      !rtmGetStopRequested(art_prs_M));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(art_prs_M, "stopping the model");
  runModel = 0;
}
...
{
  int argc = 4;
  char *argv[4] = { "art_prs", "-w", "-port", "17725" };

  my_android_init(pEnv, pObj, pArgv, pMainActivity);
  rtmSetErrorStatus(art_prs_M, 0);
