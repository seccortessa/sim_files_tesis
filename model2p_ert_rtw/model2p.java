...
public class model2p extends AppCompatActivity implements OnFragmentInteractionListener{
    ...
    private model2p thisClass;
    private final Thread BgThread = new Thread() {
    @Override
    public void run() {
            String argv[] = new String[] {"MainActivity","model2p"};
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Log.e("model2p.updateTextViewById", ex.getLocalizedMessage());
                }
            }
        });
    }
...
timerZoomButton.start();
    ...
    private native int naMain(String[] argv, model2p pThis);
    private native void naOnAppStateChange(int state);
    static {
        System.loadLibrary("model2p");
    }
