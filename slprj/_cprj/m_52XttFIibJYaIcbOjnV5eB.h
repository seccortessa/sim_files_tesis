#ifndef __52XttFIibJYaIcbOjnV5eB_h__
#define __52XttFIibJYaIcbOjnV5eB_h__

/* Include files */
#include "simstruc.h"
#include "rtwtypes.h"
#include "multiword_types.h"
#include "slexec_vm_zc_functions.h"
#include "slexec_vm_simstruct_bridge.h"
#include "sl_sfcn_cov/sl_sfcn_cov_bridge.h"

/* Type Definitions */
#ifndef struct_tag_BnecrUtJbCphGROG5igz1E
#define struct_tag_BnecrUtJbCphGROG5igz1E

struct tag_BnecrUtJbCphGROG5igz1E
{
  int32_T isInitialized;
};

#endif                                 /*struct_tag_BnecrUtJbCphGROG5igz1E*/

#ifndef typedef_codertarget_internal_androidBLEReceive
#define typedef_codertarget_internal_androidBLEReceive

typedef struct tag_BnecrUtJbCphGROG5igz1E codertarget_internal_androidBLEReceive;

#endif                                 /*typedef_codertarget_internal_androidBLEReceive*/

#ifndef struct_tag_R92EHBRJBtqsluQDZ25dkE
#define struct_tag_R92EHBRJBtqsluQDZ25dkE

struct tag_R92EHBRJBtqsluQDZ25dkE
{
  char_T f1[7];
};

#endif                                 /*struct_tag_R92EHBRJBtqsluQDZ25dkE*/

#ifndef typedef_cell_wrap
#define typedef_cell_wrap

typedef struct tag_R92EHBRJBtqsluQDZ25dkE cell_wrap;

#endif                                 /*typedef_cell_wrap*/

#ifndef struct_tag_SxrHHHk4bnX1dNNA6cbOrD
#define struct_tag_SxrHHHk4bnX1dNNA6cbOrD

struct tag_SxrHHHk4bnX1dNNA6cbOrD
{
  char_T f1[8];
  char_T f2[6];
  char_T f3[6];
  char_T f4[4];
  char_T f5[11];
  char_T f6[7];
  char_T f7[6];
  char_T f8[7];
  char_T f9[2];
  real_T f10;
};

#endif                                 /*struct_tag_SxrHHHk4bnX1dNNA6cbOrD*/

#ifndef typedef_cell
#define typedef_cell

typedef struct tag_SxrHHHk4bnX1dNNA6cbOrD cell;

#endif                                 /*typedef_cell*/

#ifndef struct_tag_bVpmb8DLX7RElZONckYfQE
#define struct_tag_bVpmb8DLX7RElZONckYfQE

struct tag_bVpmb8DLX7RElZONckYfQE
{
  char_T f1[4];
  char_T f2[6];
};

#endif                                 /*struct_tag_bVpmb8DLX7RElZONckYfQE*/

#ifndef typedef_b_cell
#define typedef_b_cell

typedef struct tag_bVpmb8DLX7RElZONckYfQE b_cell;

#endif                                 /*typedef_b_cell*/

#ifndef struct_tag_sItaFpN5TUuzOUybxGaYdB
#define struct_tag_sItaFpN5TUuzOUybxGaYdB

struct tag_sItaFpN5TUuzOUybxGaYdB
{
  char_T f1[8];
};

#endif                                 /*struct_tag_sItaFpN5TUuzOUybxGaYdB*/

#ifndef typedef_b_cell_wrap
#define typedef_b_cell_wrap

typedef struct tag_sItaFpN5TUuzOUybxGaYdB b_cell_wrap;

#endif                                 /*typedef_b_cell_wrap*/

#ifndef struct_tag_dK05fgDE5TQ2s3o7OZp84G
#define struct_tag_dK05fgDE5TQ2s3o7OZp84G

struct tag_dK05fgDE5TQ2s3o7OZp84G
{
  char_T f1[6];
  char_T f2[6];
};

#endif                                 /*struct_tag_dK05fgDE5TQ2s3o7OZp84G*/

#ifndef typedef_s_dK05fgDE5TQ2s3o7OZp84G
#define typedef_s_dK05fgDE5TQ2s3o7OZp84G

typedef struct tag_dK05fgDE5TQ2s3o7OZp84G s_dK05fgDE5TQ2s3o7OZp84G;

#endif                                 /*typedef_s_dK05fgDE5TQ2s3o7OZp84G*/

#ifndef struct_tag_UX05GARjqsuR9QofOrwOSD
#define struct_tag_UX05GARjqsuR9QofOrwOSD

struct tag_UX05GARjqsuR9QofOrwOSD
{
  char_T f1[27];
  char_T f2[21];
  char_T f3[26];
};

#endif                                 /*struct_tag_UX05GARjqsuR9QofOrwOSD*/

#ifndef typedef_c_cell
#define typedef_c_cell

typedef struct tag_UX05GARjqsuR9QofOrwOSD c_cell;

#endif                                 /*typedef_c_cell*/

#ifndef struct_tag_uo7PCp2pQzQ8uFrW43NfEH
#define struct_tag_uo7PCp2pQzQ8uFrW43NfEH

struct tag_uo7PCp2pQzQ8uFrW43NfEH
{
  char_T f1[28];
  char_T f2[22];
  char_T f3[35];
  char_T f4[26];
  char_T f5[36];
  char_T f6[39];
};

#endif                                 /*struct_tag_uo7PCp2pQzQ8uFrW43NfEH*/

#ifndef typedef_d_cell
#define typedef_d_cell

typedef struct tag_uo7PCp2pQzQ8uFrW43NfEH d_cell;

#endif                                 /*typedef_d_cell*/

#ifndef struct_tag_DyxoIdtJXNTXJqriOfucUD
#define struct_tag_DyxoIdtJXNTXJqriOfucUD

struct tag_DyxoIdtJXNTXJqriOfucUD
{
  cell_wrap _data;
};

#endif                                 /*struct_tag_DyxoIdtJXNTXJqriOfucUD*/

#ifndef typedef_s_DyxoIdtJXNTXJqriOfucUD
#define typedef_s_DyxoIdtJXNTXJqriOfucUD

typedef struct tag_DyxoIdtJXNTXJqriOfucUD s_DyxoIdtJXNTXJqriOfucUD;

#endif                                 /*typedef_s_DyxoIdtJXNTXJqriOfucUD*/

#ifndef struct_tag_zfgNabHa7yr9YEVq4jXMnE
#define struct_tag_zfgNabHa7yr9YEVq4jXMnE

struct tag_zfgNabHa7yr9YEVq4jXMnE
{
  cell _data;
};

#endif                                 /*struct_tag_zfgNabHa7yr9YEVq4jXMnE*/

#ifndef typedef_s_zfgNabHa7yr9YEVq4jXMnE
#define typedef_s_zfgNabHa7yr9YEVq4jXMnE

typedef struct tag_zfgNabHa7yr9YEVq4jXMnE s_zfgNabHa7yr9YEVq4jXMnE;

#endif                                 /*typedef_s_zfgNabHa7yr9YEVq4jXMnE*/

#ifndef struct_tag_IXDPg4TguZyNIZe3sGIUmH
#define struct_tag_IXDPg4TguZyNIZe3sGIUmH

struct tag_IXDPg4TguZyNIZe3sGIUmH
{
  b_cell _data;
};

#endif                                 /*struct_tag_IXDPg4TguZyNIZe3sGIUmH*/

#ifndef typedef_s_IXDPg4TguZyNIZe3sGIUmH
#define typedef_s_IXDPg4TguZyNIZe3sGIUmH

typedef struct tag_IXDPg4TguZyNIZe3sGIUmH s_IXDPg4TguZyNIZe3sGIUmH;

#endif                                 /*typedef_s_IXDPg4TguZyNIZe3sGIUmH*/

#ifndef struct_tag_fluQg0iJJtKg8PJrnUx9TG
#define struct_tag_fluQg0iJJtKg8PJrnUx9TG

struct tag_fluQg0iJJtKg8PJrnUx9TG
{
  b_cell_wrap _data;
};

#endif                                 /*struct_tag_fluQg0iJJtKg8PJrnUx9TG*/

#ifndef typedef_s_fluQg0iJJtKg8PJrnUx9TG
#define typedef_s_fluQg0iJJtKg8PJrnUx9TG

typedef struct tag_fluQg0iJJtKg8PJrnUx9TG s_fluQg0iJJtKg8PJrnUx9TG;

#endif                                 /*typedef_s_fluQg0iJJtKg8PJrnUx9TG*/

#ifndef struct_tag_2Otzot4aR0pW68wjnUjAj
#define struct_tag_2Otzot4aR0pW68wjnUjAj

struct tag_2Otzot4aR0pW68wjnUjAj
{
  s_dK05fgDE5TQ2s3o7OZp84G _data;
};

#endif                                 /*struct_tag_2Otzot4aR0pW68wjnUjAj*/

#ifndef typedef_s_2Otzot4aR0pW68wjnUjAj
#define typedef_s_2Otzot4aR0pW68wjnUjAj

typedef struct tag_2Otzot4aR0pW68wjnUjAj s_2Otzot4aR0pW68wjnUjAj;

#endif                                 /*typedef_s_2Otzot4aR0pW68wjnUjAj*/

#ifndef struct_tag_6khVZLlzg3270lst7fcoXG
#define struct_tag_6khVZLlzg3270lst7fcoXG

struct tag_6khVZLlzg3270lst7fcoXG
{
  c_cell _data;
};

#endif                                 /*struct_tag_6khVZLlzg3270lst7fcoXG*/

#ifndef typedef_s_6khVZLlzg3270lst7fcoXG
#define typedef_s_6khVZLlzg3270lst7fcoXG

typedef struct tag_6khVZLlzg3270lst7fcoXG s_6khVZLlzg3270lst7fcoXG;

#endif                                 /*typedef_s_6khVZLlzg3270lst7fcoXG*/

#ifndef struct_tag_Q9exWnM4P2vFrlJou54v4D
#define struct_tag_Q9exWnM4P2vFrlJou54v4D

struct tag_Q9exWnM4P2vFrlJou54v4D
{
  d_cell _data;
};

#endif                                 /*struct_tag_Q9exWnM4P2vFrlJou54v4D*/

#ifndef typedef_s_Q9exWnM4P2vFrlJou54v4D
#define typedef_s_Q9exWnM4P2vFrlJou54v4D

typedef struct tag_Q9exWnM4P2vFrlJou54v4D s_Q9exWnM4P2vFrlJou54v4D;

#endif                                 /*typedef_s_Q9exWnM4P2vFrlJou54v4D*/

#ifndef typedef_InstanceStruct_52XttFIibJYaIcbOjnV5eB
#define typedef_InstanceStruct_52XttFIibJYaIcbOjnV5eB

typedef struct {
  SimStruct *S;
  codertarget_internal_androidBLEReceive sysobj;
  boolean_T sysobj_not_empty;
  void *emlrtRootTLSGlobal;
  int8_T (*b_y0)[7];
  uint8_T *b_y1;
} InstanceStruct_52XttFIibJYaIcbOjnV5eB;

#endif                                 /*typedef_InstanceStruct_52XttFIibJYaIcbOjnV5eB*/

/* Named Constants */

/* Variable Declarations */

/* Variable Definitions */

/* Function Declarations */

/* Function Definitions */
extern void method_dispatcher_52XttFIibJYaIcbOjnV5eB(SimStruct *S, int_T method,
  void* data);

#endif
